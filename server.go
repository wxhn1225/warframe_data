...
func main() {
	...
	fmt.Println("   - factions_data.json")
	fmt.Println("   - enemy_data/ 目录 (包含所有敌人JSON文件)")
...
func loadFactionsData() {
	data, err := ioutil.ReadFile("factions_data.json")
	if err != nil {
		fmt.Println("❌ 未找到 factions_data.json，请先运行 analyze_factions.py")
		return
	}
...
func handleEnemy(w http.ResponseWriter, r *http.Request) {
	...
	// 构建文件路径
	filePath := filepath.Join("enemy_data", path+".json")
